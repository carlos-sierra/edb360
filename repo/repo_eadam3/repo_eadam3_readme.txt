...
4. Execute repo/repo_eadam3/repo_eadam3_create.sql connecting as SYS or DBA. When asked,
   pass as parameter the eadam3 repository's owner (user created on prior step).
   
   # cd edb360-master
   # sqlplus / as sysdba
   SQL> @repo/repo_eadam3/repo_eadam3_create.sql
...
5. Create an edb360 user following instructions on repo_edb360_readme.txt Repo Create
   step 1. This step 5 and following step 6 are optional but highly recommended, else
   you may get ORA-600 documented on bug 25802477.

6. Clone the eadam3 repository (external tables) into an edb360 repository (heap tables).
   Execute repo/repo_eadam3/repo_eadam3_clone.sql connecting as SYS or DBA. After this
   step you will have two usable repositories: eadam3 and edb360.

7. Configure edb360 to access edb360 (or eadam3) repository instead of base views: either 
   modify sql/edb360_00_config.sql or sql/custom_config_01.sql and set tool_repo_user.
   
   DEF tool_repo_user = 'edb360'; 

8. Execute edb360 as usual. If tool_repo_user was set on sql/edb360_00_config.sql then
...
1. Once edb360 completes you can drop the eadam3 repository, and reset tool_repo_user on
   sql/edb360_00_config.sql or sql/custom_config_01.sql.

   # cd edb360-master
   # sqlplus / as sysdba
   SQL> @repo/repo_eadam3/repo_eadam3_drop.sql
