...
SPO 9995_&&common_prefix._ash_driver.sql;
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- one node
      l_standard_filename := 'ashrpt_'||i.instance_number||'_'||j.bid||'_'||j.eid;
      ...
      IF l_instances > 1 AND '&&db_version.' >= '11' THEN
        l_standard_filename := 'ashrpt_rac_'||j.bid||'_'||j.eid;
        l_spool_filename := '&&common_prefix._'||l_standard_filename;
        put_line('COL hh_mm_ss NEW_V hh_mm_ss NOPRI FOR A8;');
        put_line('SELECT TO_CHAR(SYSDATE, ''HH24:MI:SS'') hh_mm_ss FROM DUAL;');
        put_line('-- update log');
...
/
SPO OFF;
@9995_&&common_prefix._ash_driver.sql;
SET SERVEROUT OFF HEAD ON PAGES &&def_max_rows.;
HOS zip -mq &&main_compressed_filename._&&file_creation_time. 9995_&&common_prefix._ash_driver.sql